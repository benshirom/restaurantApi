...
exports.OrderCtrl = {
  ...
  addItemToOrder: async (req, res) => {
    ...
    try {
      let { orderId, itemMenuId, price } = req.params;

      let item = new itemOrderModel(req.body);
      item.itemMenuId = itemMenuId
      await item.save()

      let order = await orderModel.updateOne({ _id: orderId }, { $push: { 'orderItems': item._id }, $inc: { 'finalPrice': price } });
      res.json(order);
    } catch (err) {
      console.log(err);
      res.status(500).json({ msg: "there error try again later", err });
  ...
  addItemsToOrder: async (req, res) => {
    //need arr that contain list of item menu id and note  
    // item is build {"itemMenuId":id num ,"note":"sdfsdf"}

    let validBody = validateItemsOrder(req.body);
    if (validBody.error) {
      return res.status(400).json({ msg: "Need to send body" });
    }
    try {
      ...
      for (let item of req.body.items) {
        ...
        await itemOrder.populate({ path: 'itemMenuId', model: 'itemmenus' })
